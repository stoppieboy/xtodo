"""This module provides the xtodo CLI."""
#xtodo/cli.py
...
from xtodo import ERRORS, __app_name__, __version__, config, database, xtodo
...
def remove(todo_id: int = typer.Argument(...),
           force: bool = typer.Option(False, "--force", "-f", help="Force deletion without confirmation.")) -> None:
...
def get_todoer() -> xtodo.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "xtodo init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return xtodo.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "xtodo init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
